// 專業化 UI：地圖工具、浮油單一標示、剖面雙軸、TWD97 座標支援
(function(){
  ...
  function addMapToolbar(){
    ...
    const mapPanel=document.querySelector('.map-panel'); const mapEl=document.getElementById('map'); if(!mapPanel||!mapEl)return;
    const toolbar=document.createElement('div'); toolbar.className='map-toolbar';
    toolbar.innerHTML='<div class="map-opacity-inline"><span>底圖透明度</span><input type="range" min="20" max="100" step="5" value="100" aria-label="底圖透明度"><strong>100%</strong></div><div class="map-crs-label">座標系：TWD97 / TM2 zone 121（EPSG:3826）</div>';
    mapPanel.insertBefore(toolbar,mapEl);
    const slider=toolbar.querySelector('input'),value=toolbar.querySelector('strong');
  ...
  window.addEventListener('load',()=>{addMapToolbar();addMapEssentials();setTimeout(()=>{consolidateLnaplMarkers();normalizeLabels();},100);const target=document.getElementById('point-detail');if(target)new MutationObserver(()=>normalizeLabels(target)).observe(target,{childList:true,subtree:true,characterData:true});});
})();
