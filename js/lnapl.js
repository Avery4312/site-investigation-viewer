...
function createHistoryView(well, container) {
    ...
    history.forEach(item => {
        ...
        col.innerHTML = `
            <div class="history-value">${Number(item.lnapl_thickness).toFixed(2)} m</div>
            <div class="history-bar-wrap"><div class="history-bar" style="height:${barHeight}px"></div></div>
    ...
    table.innerHTML = `
        <thead><tr><th>日期</th><th>地下水位深度 (m)</th><th>表觀浮油厚度 (m)</th><th>TPH (mg/L)</th></tr></thead>
        <tbody>${history.map(item => `
            <tr>
                <td>${item.date}</td>
                <td>${Number(item.water_depth).toFixed(2)}</td>
                <td class="${Number(item.lnapl_thickness) > 0 ? "history-lnapl-cell" : ""}">${Number(item.lnapl_thickness).toFixed(2)}</td>
                <td>${Number(item.tph).toFixed(2)}</td>
            </tr>`).join("")}</tbody>`;
...
showMonitoringWellDetail = function (rawWell) {
    ...
    const waterElevation = typeof well.ground_elevation === "number" && typeof well.water_depth === "number"
        ? well.ground_elevation - well.water_depth : null;
    const lnaplElevation = well.has_lnapl && typeof well.ground_elevation === "number" && typeof well.lnapl_depth === "number"
        ? well.ground_elevation - well.lnapl_depth : null;

    const info = document.createElement("div");
    info.className = "well-info-grid";
    const item = (label, value, extraClass = "") => `
        <div class="well-info-item ${extraClass}"><span class="well-info-label">${label}</span><strong>${value}</strong></div>`;
    let html = "";
...
// 在地圖上補上 Demo v2 狀態圖例與 MW02 浮油 Icon。
window.addEventListener("load", () => {
    const legend = L.control({ position: "bottomleft" });
    legend.onAdd = function () {
        const div = L.DomUtil.create("div", "map-status-legend");
        div.innerHTML = `
            <div class="map-legend-title">調查點圖例</div>
            <div><span class="legend-symbol soil"></span>土壤調查點</div>
            <div><span class="legend-symbol well"></span>地下水監測井</div>
            <div><span class="legend-symbol oil">◆</span>發現浮油</div>`;
        return div;
    };
    legend.addTo(map);
    ...
    fetch("data/samples.json")
        ...
        .then(data => {
            ...
            if (!mw02) return;
            const oilMarker = L.marker([mw02.latitude, mw02.longitude], {
                icon: L.divIcon({
                    className: "lnapl-map-marker",
                    html: `<div class="lnapl-map-icon" title="MW02 發現浮油">◆</div>`,
                    iconSize: [18, 18],
                    iconAnchor: [-10, 18]
                }),
                interactive: false
